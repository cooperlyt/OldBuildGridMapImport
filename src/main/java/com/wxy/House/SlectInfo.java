...
public class SlectInfo {
    ...
    public static ResultSet evaluateCorporation(Statement sD,String IDorNo) throws SQLException{

        if (IDorNo!=null && !IDorNo.equals("")) {
            ResultSet hs = sD.executeQuery("select * from EvaluateCorporation where ID='" + IDorNo + "'");
            //System.out.println("select * from OwnerInfo where ID='" + IDorNo + "'");
            hs.last();
            int rsCount = hs.getRow();
            //System.out.println("rsCount--"+rsCount);
            if (rsCount > 0) {
                ...
                hs = sD.executeQuery("select * from EvaluateCorporation where No='" + IDorNo + "'");
                //System.out.println("select * from OwnerInfo where No='" + IDorNo + "'");
                hs.last();
                rsCount = hs.getRow();
                // System.out.println("rsCount--"+rsCount);
